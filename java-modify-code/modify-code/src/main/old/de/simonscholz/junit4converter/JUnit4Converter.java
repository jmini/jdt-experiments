...
public class JUnit4Converter implements ICompilationUnitModifier {
    ...
    public void modifyCompilationUnit(CompilationUnit astRoot, IProgressMonitor monitor) throws JavaModelException, CoreException, BadLocationException {
        ...
        if (modifiedDocument) {
            ICompilationUnit adapter = (ICompilationUnit) astRoot.getJavaElement()
    ...
    protected void saveChanges(ICompilationUnit cu, IProgressMonitor monitor, final ASTRewrite rewriter, ImportRewrite importRewrite) throws CoreException, JavaModelException, BadLocationException {
    ...
    protected void convertTestMethods(final AST ast, final ASTRewrite rewriter, final ImportRewrite importRewrite,
        ...
        for (MethodDeclaration methodDeclaration : methods) {
            ...
            if (fullyQualifiedName.toLowerCase()
                    .startsWith(TEST_METHOD_PREFIX) && isPublicMethod(methodDeclaration)) {
    ...
    protected void removeSuperCall(ASTRewrite rewriter, MethodDeclaration methodDeclaration) {
        List<?> statements = methodDeclaration.getBody()
                .statements();
        for (Object object : statements) {
            if (object instanceof Statement) {
                Statement superCall = (Statement) object;
                String superCallAsString = superCall.toString()
                        .trim();
                String toBeReplacedSuperCall = "super." + methodDeclaration.getName()
    ...
    protected void createMarkerAnnotation(AST ast, ASTRewrite rewriter, MethodDeclaration methodDeclaration, String annotationName) {
